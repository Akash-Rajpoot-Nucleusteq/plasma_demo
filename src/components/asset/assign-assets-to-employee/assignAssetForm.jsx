...
import { LabelAndInputField, SubmitButton } from '../../common-components/InputComponents'
...
export default function AssignAssetForm({ assetData }) {
  ...
  useEffect(() => {
    if (assetData) {
      setFormData((prevData) => ({
        ...prevData,
        assetType: assetData.assetType !== undefined && assetData.assetType !== null && assetData.assetType !== '' ? assetData.assetType : '',
        assetOs: assetData.assetOs !== undefined && assetData.assetOs !== null && assetData.assetOs !== '' ? assetData.assetOs : '',
        assetProvidedBy: assetData.assetProvidedBy !== undefined && assetData.assetProvidedBy !== null && assetData.assetProvidedBy !== '' ? assetData.assetProvidedBy : '',
        assetId: assetData.assetId !== undefined && assetData.assetId !== null && assetData.assetId !== '' ? assetData.assetId : '',
      }))
    }
    if (selectedEmployee) {
      const selectedEmpData = employeeList.find((emp) => emp.employeeId === selectedEmployee);
      setFormData((prevData) => ({
        ...prevData,
        employeeClientPartner: selectedEmpData.employeeClientPartner !== undefined && selectedEmpData.employeeClientPartner !== null ? selectedEmpData.employeeClientPartner : '',
        employeeFirstName: selectedEmpData.employeeFirstName !== undefined && selectedEmpData.employeeFirstName !== null ? selectedEmpData.employeeFirstName : '',
        employeeLastName: selectedEmpData.employeeLastName !== undefined && selectedEmpData.employeeLastName !== null ? selectedEmpData.employeeLastName : '',
        employeeId: selectedEmpData.employeeId !== undefined && selectedEmpData.employeeId !== null ? selectedEmpData.employeeId : '',
        employeeWorkLocation: selectedEmpData.employeeWorkLocation !== undefined && selectedEmpData.employeeWorkLocation !== null ? selectedEmpData.employeeWorkLocation : '',
        employeeWorkType: selectedEmpData.employeeWorkType !== undefined && selectedEmpData.employeeWorkType !== null ? selectedEmpData.employeeWorkType : '',
      }));
    }
  ...
  function checkForError() {
    setFormError({
      ...formError,
      errorEmployee: InputValidator.isObjectEmpty(formData) && 'Select Employee',
    })
  }

  function setEmployee(event) {
    console.log('from setemployee()', event.target.value);
  ...
  function setEmployeeFromDropdown(value) {
    ...
    console.log('value.employeeFirstName:', value.employeeFirstName);
  ...
  const onChange = (e) => {
    console.log('from onchage: ', e.target.value);
      ...
      <div className='card-body'>
                  ...
                  <Form.Group controlId="employeeName">
                    ...
                    <div className="overflow-auto" style={{ maxHeight: '150px' }}>
                      {showOptions && value &&
                        employeeList
                          .filter(item =>
                            `${item.employeeFirstName} ${item.employeeLastName}`.toLowerCase().startsWith(value.toLowerCase()) && item.title !== value)
                          .map((emp, index) => (
                            <div key={index} onClick={() => setEmployeeFromDropdown(emp)} className='form-control'>
              ...
              <Row>
                <Col md={6}>
                  <Form.Group controlId="employeeName">
                    ...
                    <Form.Control as="select" onChange={setEmployee} value={formData.employeeId || ''}>
                    ...
                    {formError.errorEmployee && <span className="text-danger small">{formError.errorEmployee}</span>}
                  </Form.Group>
                </Col>
                <Col md={6}>
        ...
        <div className="d-flex justify-content-between align-items-center">
          <div className="d-flex">
            <SubmitButton buttonText={'Assign Asset'} />
          </div>
        </div>
      </div>
    </Form>
